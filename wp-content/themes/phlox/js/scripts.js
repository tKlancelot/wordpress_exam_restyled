...
;(function($, window, document, undefined){
    ...
    $.fn.AuxinCarouselNavigation = function( $scope ){
        ...
        $scope.find('.aux-carousel-navigation').each( function() {
            var $this   = $(this),
                $target = $( $this.data('target') ).find('.master-carousel')[0];

            if ( !$target || !AuxinCarousel ) {
                return ;
            }
            
            var instance = AuxinCarousel[$target.getAttribute('data-element-id')];

            $this.find('.aux-prev').click( { action: 'prev' }, $.proxy( instance._controlCarousel, instance ) );

            $this.find('.aux-next').click( { action: 'next' }, $.proxy( instance._controlCarousel, instance ) );
